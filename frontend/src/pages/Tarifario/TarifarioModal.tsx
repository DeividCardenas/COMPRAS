...
const TarifarioModal: React.FC<TarifarioModalProps> = ({ idTarifario, onClose }) => {
  ...
  const fetchTarifarioData = useCallback(async () => {
    if (!idTarifario) return;
    setLoading(true);
    try {
      const filters = buildFilters();
      const tarifarioResponse = await fetchTarifarioPorId(idTarifario, currentPage, itemsPerPage, filters);
  ...
  }, [idTarifario, currentPage, itemsPerPage, buildFilters]);
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-sky-900 rounded-lg shadow-lg max-w-6xl w-full max-h-full overflow-auto p-6 relative">
        <button
          onClick={onClose}
